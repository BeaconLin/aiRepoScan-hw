...
| **扫描结果导出** | 将扫描告警及标注信息导出为 Excel；已标注条目携带标注结论、原因、标注人、时间等 | 已实现（Web 端客户端导出） |
...
| **扫描结果服务端导出** | 大数据量场景由后端生成 `.xlsx` 并返回下载链接（见 3.8.3 方案 B） | 规划中 |
...
| **导出扫描结果** | 见 3.8：当前复用 `GET scan-results` 分页聚合；可选 `GET scan-results/export` | S3 + 标注表 |
...
| **边界** | 仅「已完成」任务可导出；导出范围默认遵循详情页当前筛选条件 |
...
| Sheet | 内容 |
|-------|------|
| **扫描结果** | 一行一条告警；扫描字段 + 标注字段（有则填，无则空） |
| **任务信息** | 任务 ID、名称、代码仓、分支、产品、导出条数、导出筛选条件、导出时间 |
...
**方案 A — 客户端导出（当前实现，推荐 MVP）**

不新增后端接口。Web 端复用已有 **`GET /api/tasks/{taskId}/scan-results`**，按 `pageSize=200` 循环分页直至 `hasNext=false`，在浏览器内用 `xlsx` 库生成文件并触发下载。

| 项 | 说明 |
|----|------|
| 依赖接口 | `GET scan-results`（1.3） |
| Query 透传 | `ruleName`、`annotation`、`reviewStatus` 与详情页筛选一致 |
| 客户端补充 | `keyword` 关键词仅在全量拉取后在浏览器侧过滤 |
| 文件名 | `{任务名称}_扫描结果_{yyyyMMdd_HHmmss}.xlsx` |
| 优点 | 零后端改动、与现有 mock/live 一致 |
| 限制 | 单任务数千条时浏览器内存与等待时间增加；超大任务建议方案 B |

**方案 B — 服务端导出（后续迭代，见 [`接口文档.md`](接口文档.md) 1.10）**

| 项 | 说明 |
|----|------|
| 方法与路径 | `GET /api/tasks/{taskId}/scan-results/export` |
| Query | 与 1.3 相同，另可选 `format=xlsx`（默认） |
...
| 实现 | 后端流式读取 S3 结果并 JOIN 标注表，Apache POI / EasyExcel 等生成 |
| 适用 | 单任务 5000+ 条、或需审计留痕的正式导出 |
...
flowchart LR
    ...
    B -->|否| C[按钮不可用 / 提示]
    B -->|是| D[点击「导出 Excel」]
    D --> E[按当前筛选分页拉取 scan-results]
    E --> F[合并为 Excel 两个 Sheet]
    F --> G[浏览器下载 xlsx]
```
...
| **前置条件** | `taskStatus === 已完成`；存在扫描结果（含筛选后非空） |
| **筛选联动** | 导出范围 = 当前「标注结果筛选」+ 规则树选中规则 + 关键词搜索（客户端） |
| **加载反馈** | 按钮 `loading`；Toast 显示拉取进度「已加载 m/n 条」 |
| **成功** | `ElMessage.success('已导出 x 条扫描结果')` |
| **失败** | 接口失败或无数据时明确错误文案 |
| **实现位置** | `TaskDetailView.vue` + `utils/scanResultExport.ts` |

#### 3.8.5 与评审、统计口径的关系

- 当前专家评审 UI 默认关闭（`EXPERT_REVIEW_ENABLED=false`），导出不含评审列；开关打开后与 3.7 列对齐。
- 若业务要求「仅导出已评审通过条目」，用户在评审筛选中选择「已通过」（`reviewStatus=1`）后再导出即可。
- 统计看板数字与导出条数可能不一致：看板为全任务聚合，导出可带筛选。
