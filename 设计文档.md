...
| 功能 | 说明 | 状态 |
|------|------|------|
| 专家评审 | 标注后待评审，专家通过/驳回方为最终确认 | 方案见 [`专家评审方案.md`](专家评审方案.md) |
...
flowchart TB
    ...
    UI --> API
    API --> TaskSvc & ResultSvc & AnnoSvc & StatSvc
    TaskSvc --> PG
    AnnoSvc --> PG
    StatSvc --> PG
    ResultSvc --> S3
...
| 人工标注 | `t_repo_scan_annotation` | 与 `warn_uuid` + `task_id` 关联 |
...
- 扫描结果 query：`pageNum`、`pageSize` 必填；可选 `ruleName`、`annotation`（`unmarked` / `0` / `1` / `2`）。
- 保存标注：`issueResult = null` 表示取消标注。
...
统计接口 `annotationDistribution.resultCode` 与 `issue_result` 映射见 [`接口文档.md`](接口文档.md) 1.5 节。
...
| 问题 | 场景 | 影响 | 解决方案 | 状态 |
|------|------|------|----------|------|
| 身份鉴权 | 生产环境 | 越权 | SSO + API 层校验 creator / 标注人 | 待接入 |
...
| 问题 | 场景 | 影响 | 解决方案 | 状态 |
|------|------|------|----------|------|
| 专家评审 | 标注需复核 | 流程不完整 | 扩展标注表 `review_status` 等，见专家评审方案 | 方案已定 |
...
| 正常 | 功能 | 保存标注 | 任务有告警 | POST annotations | `t_repo_scan_annotation` 正确写入 |
| 权限 | 安全 | 他人标注 | 告警已被他人标注 | 其他用户修改 | 403 |
| 正常 | 功能 | 取消标注 | 本人已标注 | issueResult=null | 标注记录删除或失效 |
...
| 层级 | 范围 |
|------|------|
| 单元 | 结果 JSON 解析、标注权限、状态机转换、统计聚合 |
| 集成 | API ↔ PostgreSQL ↔ S3；启动扫描与报告回写 |
| E2E | Web 创建任务 → 标注 → 看板；与后端 live 环境联调 |

---

*文档版本：全栈视角；数据库表结构以 PostgreSQL schema `ai_code_repo_scan` 为准。*
